...
\footnote{S.Lehtola, \textsl{Fully numerical Hartree-Fock and density functional
    calculations. I. Atoms}, Int. J. Quantum Chem. \doi{10.1002/qua.25945}} are retrieved
...
\item \textbf{DFT}
\begin{description}
\item[Format:] \textbf{dft}  $[\;c_1\;]\;\; [\;c_2\;]\;\; [\;c_3\;]$ \ldots \\
\begin{itemize}
\item[$c_i:$] specifies the type of DFT exchange-correlation functional(s) to be used in
  ...
  be used, e.g. lda\_x, lda\_c\_wigner, lda\_c\_xalpha, lda\_c\_vwn, gga\_x\_gam,
  gga\_c\_gam, etc (see commons8.f90 for the full list of supported functionals or use
  \texttt{lxcctl} utility to generate the list of available functionals and also to test
  them).
...
\textsl{lda\_c\_xalpha} one.
% HF orbitals: dft lda xalpha=2/3    == dft LDA_X 
%              dft lda xalpha=1-2/3  == dft lda_c_xalpha
%              dft lda xalpha=1      == dft LDA_X + lda_c_xalpha

\item $b88$ -- the Becke exchange potential; this functional is equivalent to
\textsl{gga\_x\_b88} one from Libxc library

  % When the Becker exchange potential is used the SFC process diverges!
  % The exchange energy calculated with HF orbitals for a set of test cases turns out to
  % be correct (see Table 1 in Becke's paper)

  % dft gga_x_b88 SCF process is divergent
  % dft gga_x_pw91 ok
  % dft gga_c_lyp  ok
  % dft gga_c_p86  ok

\item $pw86$ -- the exchange functional of Perdew and Wang; this functional is equivalent to
\textsl{gga\_x\_pw86} one from Libxc library

\item $pw91$ -- the exchange functional of Perdew and Wang; this functional is equivalent to
\textsl{gga\_x\_pw91} one from Libxc library

\item $lyp$ -- the correlation potential of Lee, Yang and Parr; this functional is equivalent to
\textsl{gga\_c\_lyp} one from Libxc library
...
\verbatiminput{../examples/kr/kr.data}
