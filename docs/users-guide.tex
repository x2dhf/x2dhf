...
\item \textbf{POTSAP}
\begin{description}
\item[Format:] \textbf{potsap} \\ When the OED method is chosen then
  this label selects a model potential due to S.Lethola.%
  \footnote{S. Lehtola, "Assessment of Initial Guesses for Self-Consistent Field
    Calculations. Superposition of Atomic Potentials: Simple yet Efficient",
    J. Chem. Theory Comput. 15, 1593 (2019). DOI: 10.1021/acs.jctc.8b01089} For a given
  atom this potential produces HF-like orbitals but it has been found useful in finding
  good starting orbitals for any molecular system.
\end{description}
...
\item The program should be easy to use provided you can start a
  ...
  Lethola (POTSAP).
  % The latter potential provides much better initial orbital estimates
