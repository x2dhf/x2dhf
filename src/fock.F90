...
! print= 95: checking (E-V) values for orbital
    ...
    do iorb1=firstOrb,lastOrb
       ...
       if (iorb1.ne.iorb)  then
          ...
          if ((mm(iorb).gt.0).and.(ilc2(iorb,iorb1)>0)) then
             call dcopy (mxsize,excp(ibexp:),ione,wk2,ione)
             call prod  (mxsize,psi(iborb1:),wk2)
...
! debug= 94: fockHF: Exchange potential contribution 
  ...
  ! ### fockTED ###
  !
  !     Calculates the Fock potential for a two electron diatomic,  e.g. the
  !     Hooke's atom (harmonium).
  !
  subroutine fockTED (iorb)
    use commons
    use params
    use discrete
    use scfshr
...
! debug= 94: fockHF: Exchange potential contribution 
  ...
  subroutine fockLXCunpol(iorb)
    ...
    if (lxcHyb) then
       ...
       ! contribution from the orbital
       call dcopy (mxsize,excp(i2b(iorb):),ione,wk3,ione)       
       call prod (mxsize,psi(i1b(iorb):),wk3) 
       call dcopy (mxsize,wk3,ione,wk4,ione)
       ...
       ! add alphaf portion of exact exchange potential 
       call dscal (mxsize,alphaf,wk4,ione)
       call dcopy (mxsize,wk4,ione,exchangeptr(ibpot:),ione)

       ! multiply coulomb/exchange potentials by f2
       call prod (mxsize,f2,wk4)
       call dcopy (mxsize,wk4,ione,fock2,ione)
    endif
...
100 continue
  ...
  subroutine fockDFT(iorb)
    ...
    integer (KIND=IPREC) :: i,iborb,iborb1,ibpot,ibpot1,iorb,iorb1
