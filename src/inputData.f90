...
subroutine inputData(ni_t,mu_t,no_t,nons_t)
  ...
  integer :: i,icompLAdd,icompLEnc,icompLExp,id1,id2,iform_t,ig,ihit,inpiexit,&
       ...
       izz1,izz2,j,mgi,mt,n,nbsym,next,nmethods,nmutot,nlabels,no,nonortho,maxflags,&
  ...
  parameter (nmethods=5,nlabels=47,maxflags=40)
  ...
  data labellc /'break','config','conv','debug','dft','exchio','fefield','fermi','fix','fixorb',&
       'fixcoul','fixexch','gauss','grid','homo','initial','inout','interp','lcao','lagra',&
       'mcsor','method','multipol','nuclei','omega','orbpot','order','potgsz','potgszg','poth3',&
       'potkh','print','prtevery','scf','scfexch','sor','sormcsor','stop','title','xalpha',&
       'potharm', 'potharm2','potharm3','pothook','out4dd','potdft','out4dft'/
...
5 continue
  ...
  if (clabel.eq.'potharm2') then
     !        label label label label label label label label label label label label label 
     !        label: potharm2
     
     !        harmonic potential omega^2(x^2+y^2)
     imethod=2     ! OED method must be used
     iharm2=1
     harm=1.0_PREC
     call inFloat(harm)
     goto 5
  ...
  if (clabel.eq.'potharm3') then
     !        label label label label label label label label label label label label label 
     !        label: potharm3
     
     !        harmonic potential omega^2(x^2+y^2+z^2)
     imethod=2     ! OED method must be used
     iharm3=1
     harm=1.0_PREC
     call inFloat(harm)
  ...
  if (clabel.eq.'potdft') then
     !        label: potdft
     !        external DFT potential formed from previously converged orbitals (HF, HFS, DFT) 
