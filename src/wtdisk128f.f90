...
subroutine wtdisk128f (cw_orb,cw_coul,cw_exch,i8tmp,r16l60,r16l1200,r16l3660,r16mxsize)
  use params
  use discret
  use memory
  use scf
  ...
  implicit none
  integer :: i,iorb1,iorb2,ioffset,iout8,j,k
  real (PREC), dimension(*) :: cw_orb,cw_coul,cw_exch

  integer*8 :: i8tmp1,i8tmp2,i8tmp3,ierr8
...
!   write (iout21,err=1000) i1b,i2b,i3b,i1e,i2e,i3e,
!   &     i1si,i2si,i3si,i1ng,i2ng,i3ng,
!   &     i1mu,i2mu,i3mu	
      

      write (iout21,formint,err=1000) (i8tmp(i),i=1,ioffset)
...
!   add orbitals
      ...
      do i=1,norb,1
         ...
         enddo
         call writea128f(iout21,i8tmp1,r16mxsize,ierr8)
         if (ierr8.ne.0) then
...
!    write(iout21,err=1020) area
      write(iout21,formfp128,err=1020) (r16l60(i),i=1,maxorb)

      do i=1,maxorb
         r16l60(i)=eng(i)
      enddo
!    write(iout21,formfp128,err=1020) eng
      write(iout21,formfp128,err=1020) (r16l60(i),i=1,maxorb)

      do i=1,maxorb*maxorb
         r16l3660(i)=engo(i)
      enddo
!    write(iout21,formfp128,err=1020) engo
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*maxorb)

      do i=1,20*maxorb
         r16l1200(i)=cmulti(i)
      enddo
!    write(iout21,formfp128,err=1020) cmulti
      write(iout21,formfp128,err=1020) (r16l1200(i),i=1,20*maxorb)

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=excdi(i)
      enddo
!    write(iout21,formfp128,err=1020) excdi
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=excqu(i)
      enddo
!    write(iout21,formfp128,err=1020) excqu
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=excoc(i)
      enddo
!    write(iout21,formfp128,err=1020) excoc
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=exche(i)
      enddo
!    write(iout21,formfp128,err=1020) exche
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=exc5(i)
      enddo
!    write(iout21,formfp128,err=1020) exc5
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=exc6(i)
      enddo
!    write(iout21,formfp128,err=1020) exc6
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=exc7(i)
      enddo
!    write(iout21,formfp128,err=1020) exc7
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))

      do i=1,maxorb*(maxorb+1)
         r16l3660(i)=exc8(i)
      enddo
!    write(iout21,formfp128,err=1020) exc8
      write(iout21,formfp128,err=1020) (r16l3660(i),i=1,maxorb*(maxorb+1))
...
!   write out Coulomb potentials
      ...
      do i=1,norb
         ...
         enddo
         i8tmp1=i2si(i)
         call writea128f(iout22,i8tmp1,r16mxsize,ierr8)
         ...
         if (iform.eq.2.or.iform.eq.3) then
            ...
            do iorb1=1,norb
               do iorb2=iorb1,norb
                  ...
                  enddo
                  i8tmp1=i3si(k)
                  call writea128f(iout23,i8tmp1,r16mxsize,ierr8)
...
! FIXME
 1020 continue
      write(*,*) 'wtdisk128f: error encountered when writing an extension to orbital file'
      write(*,*) '      '
      stop 'wtdisk128f'
 1050 format(1x,'... writing functions to disk (r128f) ...')
 1070 format(//1x,'error! can not write data to disk'//)     
 1080 format(//1x,'error! can not write data to disk'//)     

    end subroutine wtdisk128f
