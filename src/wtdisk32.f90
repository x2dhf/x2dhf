...
! c     Writes orbitals, potenials, Lagrange multipliers and multipole
! c     expansion coefficients to a disk file in an unformatted form:
! c     integers are 4 and reals are 8-byte long.

module wtdisk32_m
  implicit none
contains
  subroutine wtdisk32 (cw_orb,cw_coul,cw_exch,i4tmp,r8l60,r8l1200,r8l3660,r8mxsize)
    use params
    use discret
    use memory
    use scf
    ...
    write (iout21,err=1000) (i4tmp(i),i=1,ioffset)
    ...
    do i=1,norb
       ...
       enddo
       call writea32(iout21,i4tmp1,r8mxsize,ierr)
       if (ierr.ne.0) then
    ...
    do i=1,norb
       ...
       enddo
       call writea32(iout22,i4tmp1,r8mxsize,ierr)
       ...
       if (iform.eq.2.or.iform.eq.3) then
          do iorb1=1,norb
             do iorb2=iorb1,norb
                ...
                enddo
                call writea32(iout23,i4tmp1,r8mxsize,ierr)
